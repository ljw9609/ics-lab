...
cc_t compute_cc(alu_t op, long_t argA, long_t argB, long_t val)
{
    ...
    if(val < 0)
        sign = TRUE;
    if((op = A_ADD)&&((argA < 0) == (argB < 0))&&((val < 0) != (argB < 0)))
        ovf = TRUE;
    if((op = A_SUB)&&((argA > 0) == (argB < 0))&&((val < 0) != (argB < 0)))
        ovf = TRUE;
...
stat_t nexti(y86sim_t *sim)
{
    ...
    long_t valA,valB,valC,valM,valE = 0;
    ...
    byte_t reg = 0;
    if((icode >= I_RRMOVL && icode <= I_ALU)||
        (icode >=I_PUSHL && icode <=I_POPL)){
    ...
    // done!
    long_t imm_num = 0;
    if((icode >= I_IRMOVL && icode <= I_MRMOVL)||
    ...
    switch (icode) {
      ...
      case I_RRMOVL:  /* 2:x regA:regB */ //done!
      ...
      case I_IRMOVL: /* 3:0 F:regB imm */ //done!
        if(!NONE_REG(regA) || !NORM_REG(regB)) return STAT_INS;
      ...
      case I_RMMOVL: /* 4:0 regA:regB imm */ //done!
        if(!NORM_REG(regA) || !NORM_REG(regB)) return STAT_INS;
        if(ifun != 0) return STAT_INS;
        valA = get_reg_val(sim->r,regA);
        ...
        if(!set_long_val(sim->m,valE,valA)){
            err_print("PC = 0x%x, Invalid instruction address: 0x%x", sim->pc,valE);
            return STAT_ADR;
        }
        sim->pc = next_pc;
        break;

      case I_MRMOVL: /* 5:0 regB:regA imm */ //done!
        if(!NORM_REG(regA) || !NORM_REG(regB)) return STAT_INS;
        if(ifun != 0) return STAT_INS;
        valC = imm_num;
        valB = get_reg_val(sim->r,regB);
      ...
      case I_PUSHL: /* A:0 regA:F */ //done!
        if(!NORM_REG(regA) || !NONE_REG(regB)) return STAT_INS;
        if(ifun != 0) return STAT_INS;
        valA = get_reg_val(sim->r,regA);
        valB = get_reg_val(sim->r,REG_ESP);
        valE = valB + (-4);
